...
public class FeedFish : MonoBehaviour {
	...
	void Update () {
		...
		v.x = mSprite.transform.position.x + (velocity * dir * Time.deltaTime)/**(1+PlayerFish.Instance.FeverTime*Player.Instance.FeverAcc)*/;
		...
		if(Distance2to3(PlayerFish.Instance.playerPosition, mSprite.transform.position) < 15
			&& PlayerFish.Instance.sizeOfFish >= sizeOfFish)
		{
