...
public class JoyStickForMouse : MonoBehaviour

{

    static private JoyStickForMouse[]   joysticks;                                                                  //a static collection of all joysticks
...
//  private float               firstDeltaTime          = .5F;
    ...
    {       
...

        {
            ...
            joysticks = (JoyStickForMouse[]) FindObjectsOfType( typeof(JoyStickForMouse) );                                 // Collect all joysticks in the game, so we can relay finger latching messages
        ...
        if(isTouched==false)
            {
				...
				guiTouchOffset = new Vector2(Input.mousePosition.x, Input.mousePosition.y);
				//defaultRect.center =  new Vector2(Input.mousePosition.x, Input.mousePosition.y);
				guiCenter = new Vector2(Input.mousePosition.x, Input.mousePosition.y);
				transform.localPosition = new Vector2((Input.mousePosition.x-50)/Screen.width, (Input.mousePosition.y-50)/Screen.height);
				guiButtonBoundary.min.x = defaultRect.x - Screen.width/8;             
       		     guiButtonBoundary.max.x = defaultRect.x + Screen.width/8;
        	    guiButtonBoundary.min.y = defaultRect.y - Screen.width/8;
         	   guiButtonBoundary.max.y = defaultRect.y + Screen.width/8;
				isTouched = true;
			...
			isTouched = false;
			//Debug.Log("MouserButton is not downd");
			//ResetJoystick();
		}
...

        {
...

           {
...

                {
                    ...
                    foreach ( JoyStickForMouse j in joysticks )                                                     // Tell other joysticks we've latched this finger
